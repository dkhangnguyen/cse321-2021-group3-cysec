...
\begin{itemize}
    ...
    \item \textbf{Club Member}: As a club member, users can vote on polls made by Officials, create a discussion thread, post or reply a comment. Member can also apply to become an Official, which can be approved by other Officials.
    \item \textbf{Club Officials}: As a club official, user will have access to a new tab called Manage, in which users can create a new voting poll or news to the News page, look at Web Statistic, and the discussion post with other officials regarding the latest voting poll result. Official can also remove a discussion post or hide a post from public (only signed-up members can see the discussion). Officials can also approve the request to become an official from Member users.
\end{itemize}
